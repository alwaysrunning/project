...
define(function(require) {
    ...
    var createHtml = function(res) {
        ...
        if (res.productOne && res.productOne.length > 0 ) {
            ...
            for (i = 0, len = cA.length; i < len; i = i + 1) {
                ...
                str += '<div class="gd-labels"></div>';
                str += '</a>';
                str += '<div class="gd-cmd ' + (item.isbuynow === '1' ? 'jBuyNow' : 'jAdd2Cart') + '" data-id="' + item.product_id + '"' + ' data-bpm="3.' + res.mark + '_productOne' + '.1.' + i + '.0.' + item.product_id + '"' + '>';
                str += '<i class="iconglobal">' + (item.isbuynow === '1' ? '&#xe612;' : '&#xe603;') + '</i>';
                str += '</div>';
                str += '</div>';
            }
            str += '</div></div>';
    ...
    var eventInit = function() {
        ...
        var lazyMore = new LazyStream('.jPage', {
            ...
            noAnyMore: '<div class="loading"></div>'
        });
    };
